...
generated quantities{
  vector[D] eta_rep; // replicated daily reproductive rate
  vector[D] psi_rep; // replicated local infection rate
  ...
  for (i in 1:D) eta_rep[i] = gamma_rng(k * y_reg[i], k ./ R[i]);
  psi_rep = rep_vector(0, D);
  for (i in 2:D) {
    if (i <= S) {
      for (j in 1:(i - 1)) {
        psi_rep[i] = psi_rep[i] + (eta_rep[i-j] * omega[j]);
      }
    } else {
      for (j in 1:S) {
        psi_rep[i] = psi_rep[i] + (eta_rep[i-j] * omega[j]);
  ...
  for (i in (D_seed+1):D) {
    ...
    y_rep[i-D_seed] = poisson_rng(mu[i] + psi_rep[i]);
  }
